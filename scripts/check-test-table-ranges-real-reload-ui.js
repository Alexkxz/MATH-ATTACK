...
(async () => {
  ...
  try {
    ...
    try {
      ...
      await page.getByRole('button', { name: '5. Vista previa' }).click(); const patchResponsePromise = page.waitForResponse(response => response.url().includes('/api/exams/') && response.request().method() === 'PATCH'); await page.locator('#prStartConfiguredTest').click(); const patchResponse = await patchResponsePromise; assert.equal(patchResponse.status(), 200);
