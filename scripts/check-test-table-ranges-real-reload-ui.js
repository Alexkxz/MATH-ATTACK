  ...
  try {
    ...
    try {
      ...
      async function openAndSelect() {
        ...
        await page.waitForSelector(`[data-test-library-card="${testId}"]`); await page.locator(`[data-test-library-card="${testId}"] .pr-test-select`).click(); await page.locator('[data-pr-section-target="programming"]').click();
      ...
      await page.reload({ waitUntil: 'domcontentloaded' }); await page.click('#tab-pruebas'); await page.evaluate(() => window.prLoadTestLibrary()); await page.waitForSelector(`[data-test-library-card="${testId}"]`); await page.locator(`[data-test-library-card="${testId}"] .pr-test-select`).click(); await page.locator('[data-pr-section-target="programming"]').click(); await page.waitForFunction(() => document.querySelector('#prRangesPreview_mult')?.textContent.includes('Tabla 8')); assert.equal(await page.locator('.pr-range-chip').count(), 1);
      ...
      await page.waitForFunction(() => document.querySelector('#prRangesPreview_mult')?.textContent.includes('6 ejercicios')); await page.reload({ waitUntil: 'domcontentloaded' }); await page.click('#tab-pruebas'); await page.evaluate(() => window.prLoadTestLibrary()); await page.waitForSelector(`[data-test-library-card="${testId}"]`); const configurationBeforeDelete = page.waitForResponse(response => response.url().includes(`/api/exams/${testId}`) && response.request().method() === 'GET'); await page.locator(`[data-test-library-card="${testId}"] .pr-test-select`).click(); await configurationBeforeDelete; await page.locator('[data-pr-section-target="programming"]').click(); await page.waitForFunction(() => document.querySelector('#prRangesPreview_mult')?.textContent.includes('6 ejercicios')); assert.equal(await page.locator('.pr-range-chip').count(), 1);
