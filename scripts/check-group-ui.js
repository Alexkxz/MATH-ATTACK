  ...
  try {
    ...
    try {
      ...
      async function loadPage() {
        ...
        await page.click('#tab-pruebas'); await page.waitForFunction(() => typeof window.prLoadTestLibrary === 'function'); await page.evaluate(() => window.prLoadTestLibrary()); await page.waitForSelector(`[data-test-library-card="${testId}"]`); await page.locator(`[data-test-library-card="${testId}"] .pr-test-select`).click(); await page.locator('[data-pr-section-target="assignments"]').click(); await page.evaluate(() => window.prLoadGroups()); await page.waitForSelector('#prGroupsList');
      }
      await loadPage();
      ...
      await page.click('#prGroupAssign'); await page.waitForFunction(() => document.querySelector('#prTestLibraryStatus')?.textContent.includes('prueba(s) disponibles'));
