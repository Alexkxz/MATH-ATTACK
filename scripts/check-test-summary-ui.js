...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.pr-summary-stop').count(), 0);
    assert.equal(await page.locator('#prTestSummary .pr-test-create').count(), 0);
    assert((await page.locator('#prTestSummary').textContent()).includes('Folio'));
