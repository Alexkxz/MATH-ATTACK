...
(async () => {
  ...
  try {
    ...
    assert(!text.includes('accountPlayerId')); assert(!text.includes('testId')); assert(!text.includes('attemptId')); assert(!text.includes('parent:')); assert(text.includes('Checkpoint')); assert(text.includes('Desconectado')); assert(text.includes('4 / 10'));
